...
void app_main(void)
{
    ...
    if (nvs_flash_init())
    {
        ...
        if (esp_partition)
        {
            ...
            nvs_flash_generate_keys(esp_partition, &nvs_sec_cfg);
        }
        ESP_ERROR_CHECK(nvs_flash_init());
    }
    const esp_partition_t *esp_partition = esp_partition_find_first(ESP_PARTITION_TYPE_DATA, ESP_PARTITION_SUBTYPE_DATA_NVS_KEYS, NULL);
    ...
    if (nvs_flash_secure_init_partition("nvs_priv", &nvs_sec_cfg))
    {
        ESP_ERROR_CHECK(nvs_flash_erase_partition("nvs_priv"));
        ESP_ERROR_CHECK(nvs_flash_secure_init_partition("nvs_priv", &nvs_sec_cfg));
    }
#endif
    nvs_handle_t nvs_handle;
    nvs_open_from_partition("nvs_priv", "store", NVS_READWRITE, &nvs_handle);
    nvs_set_str(nvs_handle, "S3_ACCESS", "AKIA2ZAB4IEWFBAKOHWX");
    nvs_set_str(nvs_handle, "S3_SECRET", "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");
    ...
    ESP_LOGI("NVS", "S3_ACCESS %s S3_SECRET %s", api_key.accessKey, api_key.secret);
    nvs_close(nvs_handle);
